...
Here $M$ is an $n$-dimensional spacetime manifold and connections are typically called gauge fields.
...
Topologically, the space $\Map(X,BG)$ of gauge fields is aspherical and its fundamental groupoid $\Pi_1(\Map(X,BG))$ is equivalent to the groupoid $\mathcal{F}//\mathcal{G}$ of principal bundles over $M$.
...
\[
Z(X) = \sum_{A \in [X,BG]} \frac{1}{|\pi_1(\Map(X,BG);A)|} \int_X A^*(\omega) \in \C^\times.
\]
\end{definition}
...
In the example of Dijkgraaf-Witten theory in a general spacetime dimension $n$, we constructed a fusion $(n-2)$-category $C$ and looked at the induced state sum with values in $(n-1)\Cat_\C$.
...
Note that since we are working with $\mathcal{O}(X)$ and not with the fundamental $\infty$-groupoid of $X$, such formal combinations are allowed to land in objects of $\mathcal{C}$ that are not $\otimes$-invertible.
...

An action functional on $X$ in this formulation, should be a functor $S: \Fun(\mathcal{O}(X), BC) \to \C^\times$.
Here we defined $S$ not only on the simple object-valued fields, so that the fusion product is correctly encoded.
Actually, we will only consider action functionals that are `functorial in spacetime'. \textcolor{red}{False! want locality/sheaflike condition (maybe there is a way to say this more categorical but we don't know, especially because sheaves on simplicial sets are not clear).}
...
\begin{enumerate}
    ...
    \item comment on non-invertible gauge theories (note that there is a problem with formulating classical gauge fields for non-invertible symmetries)
\end{enumerate}}
